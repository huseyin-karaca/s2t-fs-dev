...
class FASTTAlternating(BaseEstimator, ClassifierMixin):
    ...
    def __init__(
        ...
        transform_lr=1e-3,
        transform_steps=100,
        transform_weight_decay=1e-4,
        transform_lambda1=0.01,
        transform_lambda2=0.01,
        random_state=42,
    ):
        ...
        self.transform_weight_decay = transform_weight_decay
        self.transform_lambda1 = transform_lambda1
    ...
    def fit(self, X, y):
        ...
        # Merge scalar transform params into t_kwargs at fit-time.
        # weight_decay is shared by linear / low_rank / nonlinear;
        # lambda1 & lambda2 are exclusive to DiagonalGating — injecting
        # them into other ctors would raise an unexpected-kwarg TypeError.
        t_kwargs = dict(self.transform_kwargs or {})
        if self.transform_type == "diagonal":
            t_kwargs.setdefault("lambda1", self.transform_lambda1)
            t_kwargs.setdefault("lambda2", self.transform_lambda2)
        else:
    ...
    def get_feature_importances(self):
        """Return feature importance derived from the transform layers."""
        if self.transform_ is None:
            return None
        
        if self.transform_type == "diagonal":
            return np.abs(self.transform_.q.detach().cpu().numpy())
        elif self.transform_type == "linear":
            # L2 norm of the input-dimensional weights
            return torch.norm(self.transform_.linear.weight, p=2, dim=0).detach().cpu().numpy()
        elif self.transform_type in ["low_rank", "nonlinear"]:
            # Feature importance driven by the input transformation matrix W1
            return torch.norm(self.transform_.w1.weight, p=2, dim=0).detach().cpu().numpy()
