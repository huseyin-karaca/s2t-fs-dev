...
class AdaSTTObjectiveLGBM:
    ...
    def __call__(self, preds, train_data):
        # LightGBM multi-class predictions are 1D arrays of shape (num_classes * num_samples,)
        # ordered by class: [class_0_sample_0...N, class_1_sample_0...N, ...]
        N = self.wer_matrix.shape[0]
        K = self.num_classes
        
        # Reshape to (K, N) and transpose to (N, K)
        ...
        # Flatten back to (num_classes * num_samples,) ordered by class to match LightGBM requirements
        flat_grad = grad.T.flatten().astype(np.float64)
        flat_hess = hess.T.flatten().astype(np.float64)
