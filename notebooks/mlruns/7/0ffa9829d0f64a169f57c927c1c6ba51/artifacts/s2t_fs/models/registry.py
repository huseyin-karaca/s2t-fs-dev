...
def resolve_nested_configs(init_args: dict) -> dict:
    """Recursively resolve nested estimator configs in init_args.

    If a value is a dict containing 'class_path', it is treated as an
    estimator spec and instantiated via instantiate_from_path.
    This enables config-driven composition (e.g., FASTTAlternating's base_selector).
    """
    resolved = {}
    for key, val in init_args.items():
        if isinstance(val, dict) and "class_path" in val:
            nested_args = resolve_nested_configs(val.get("init_args", {}))
            resolved[key] = instantiate_from_path(val["class_path"], **nested_args)
            logger.debug(f"Nested estimator '{key}' resolved from {val['class_path']}")
        else:
            resolved[key] = val
    return resolved
