...
def hpt_single_model(cfg, config_path=None, preloaded_data=None):
    ...
    if not model_hpt_space:
        logger.bind(category="Inner-Run").info(
            f"No hyperparameter space provided for {model_name}. Skipping Optuna HPT - evaluating instantly."
        )
        search = None
        best_estimator = model_instance
    else:
        logger.bind(category="Inner-Run").info(
            f"Hyperparameter tuning of {model_name}: ({n_trials} trials, parallel trials)"
        )
    ...
    with mlflow.start_run(run_name=f"Model_Tuning_{model_name}", nested=True):
        ...
        # Log custom test metric since search wrapper is not always present
        mlflow.log_metric("custom_test_wer", wer_best_model)

        logger.bind(category="HPT-Detail").success(
            f"Execution of {model_name} completed. Final Test WER: {wer_best_model:.4f}"
        )
