...
class FASTTBoosted(BaseEstimator, ClassifierMixin):
    ...
    def get_gating_weights(self):
        """Return per-round gating vectors (only for diagonal transform)."""
        if self.transform_type != "diagonal" or self.model_ is None:
            return None
        weights = []
        for transform in self.model_.transforms:
            weights.append(transform.q.detach().cpu().numpy())
        return weights
