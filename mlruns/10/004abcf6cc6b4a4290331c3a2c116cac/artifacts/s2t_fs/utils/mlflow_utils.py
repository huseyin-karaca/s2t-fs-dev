...
def compute_and_log_margin(results, target_model_names, run_id):
    """
    Compute the WER margin between our model(s) and the best competitor,
    and log the results to an MLflow run.

    Margin = best_competitor_wer - our_best_wer (positive means we win).

    Parameters
    ----------
    results : dict[str, float]
        Mapping of model_name → best_wer.
    target_model_names : str or list[str]
        Name(s) of our model(s) in the results dict.
    run_id : str
    ...
    missing = [n for n in target_model_names if n not in results]
    if missing:
        logger.error(f"Target model(s) not found in results: {missing}")
        return None

    # Our best WER = min across target models
    our_results = {n: results[n] for n in target_model_names}
    best_our_name = min(our_results, key=our_results.get)
    our_best_wer = our_results[best_our_name]

    # Competitors = everyone not in target list
    competitors = {n: w for n, w in results.items() if n not in target_model_names}
    ...
    client = MlflowClient()
    client.log_metric(run_id, "our_best_wer", our_best_wer)
    ...
    summary = {
        ...
        "best_target_model": best_our_name,
        "our_best_wer": our_best_wer,
        "our_all_wers": our_results,
    ...
    logger.bind(category="Margin").success(
        f"Our Best ({best_our_name}): {our_best_wer:.4f} | "
