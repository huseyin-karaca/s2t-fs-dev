    ...
    def __init__(
        ...
        for _ in range(num_rounds):
            ...
            if base_estimator is None:
                ...
                import importlib
                cp = base_estimator.get("class_path", base_estimator.get("class_path_"))
                parts = cp.split(".")
                module_name = ".".join(parts[:-1])
                class_name = parts[-1]
                module = importlib.import_module(module_name)
                EstimatorClass = getattr(module, class_name)
                init_args = base_estimator.get("init_args", base_estimator.get("init_args_", {})).copy()
                
                if "in_features" in EstimatorClass.__init__.__code__.co_varnames:
                    init_args["in_features"] = out_dim
                elif "input_dim" in EstimatorClass.__init__.__code__.co_varnames:
                    init_args["input_dim"] = out_dim
                
                if "tree_dim" in EstimatorClass.__init__.__code__.co_varnames:
                    init_args["tree_dim"] = num_classes
                elif "output_dim" in EstimatorClass.__init__.__code__.co_varnames:
                    init_args["output_dim"] = num_classes
                    
                tree = EstimatorClass(**init_args)
