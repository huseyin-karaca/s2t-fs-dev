...
def hpt_single_model(
    ...
    if not model_hpt_space:
        ...
        # Always use parallel trials — even when called from Level 2.
        # While sequential trials give TPE slightly better Bayesian feedback,
        # the wall-clock savings from parallelism far outweigh this, especially
        # when some models finish quickly and leave pool workers idle.
        n_jobs = -1
        logger.bind(category="Inner-Run").info(
            f"HPT of {model_name}: {n_trials} trials (parallel trials)"
        )
