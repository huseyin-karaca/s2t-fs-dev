...
def hpt_single_model(
    ...
    if not model_hpt_space:
        ...
        # n_jobs=1 when nested → sequential trials so TPE builds on previous results
        # n_jobs=-1 when standalone → parallel trials for speed
        n_jobs = 1 if parallel_mode else -1
        mode_label = "sequential trials, parallel models" if parallel_mode else "parallel trials"
        logger.bind(category="Inner-Run").info(
            f"HPT of {model_name}: {n_trials} trials ({mode_label})"
        )
